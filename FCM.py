...
def main():
        ...
        try:
            #--------------Lord image file--------------  
            img= cv2.imread("example3.bmp", cv2.IMREAD_GRAYSCALE) # cf. 8bit image-> 0~255

            #--------------Clustering--------------  
            cluster = FCM(img, image_bit=8, n_clusters=4, m=2, epsilon=0.05, max_iter=-1)
